...
tape( 'the function has an arity of 6', function test( t ) {
	t.strictEqual( gcusumpw.length, 6, 'has expected arity' );
	t.end();
});
...
tape( 'the function computes the cumulative sum (accessors)', function test( t ) {
	...
	gcusumpw( x.length, 0.0, toAccessorArray( x ), 1, toAccessorArray( y ), 1 );
	expected = [
		1.0,
		3.0,
		6.0,
	...
	gcusumpw( x.length, 10.0, toAccessorArray( x ), 1, toAccessorArray( y ), 1 );
	expected = [
		11.0,
		13.0,
		16.0,
...
tape( 'the function returns a reference to the output array', function test( t ) {
	...
	out = gcusumpw( x.length, 0.0, x, 1, y, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', function test( t ) {
	...
	gcusumpw( -1, 0.0, x, 1, y, 1 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );

	gcusumpw( 0, 0.0, x, 1, y, 1 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );
...
tape( 'the function supports an `x` stride (accessors)', function test( t ) {
	...
	gcusumpw( 3, 0.0, toAccessorArray( x ), 2, toAccessorArray( y ), 1 );
...
tape( 'the function supports a `y` stride (accessors)', function test( t ) {
	...
	gcusumpw( 3, 0.0, toAccessorArray( x ), 1, toAccessorArray( y ), 2 );
